...
import { tryParseFunction, tryParseRegularExpressionLiteral, tryParseStringLiteral } from "@plexcord-companion/plexcord-ast-parser";
        ...
        function walk(node: Node) {
            ...
            if (isCallExpression(node) && finds.includes(type = node.expression.getText())) {
                ...
                lenses.push(new CodeLens(range, {
                    title: "View Module",
                    command: "plexcord-companion.extractFind",
                    arguments: [
                        {
                            type: "extract",
                            data: {
