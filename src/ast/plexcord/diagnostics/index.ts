...
const diagnosticCollection = languages.createDiagnosticCollection("plexcord-companion");
...
async function updateDiagnosticsImmediately(e: Uri) {
    ...
    const doc = new PlexcordAstParser(decoder.decode(bytes), e.fsPath);
...
async function makePatchDiagnostic(doc: PlexcordAstParser): Promise<Diagnostic[]> {
    try {
        return (
...
async function makeFindDiagnostic(doc: PlexcordAstParser): Promise<Diagnostic[]> {
    try {
        return (
