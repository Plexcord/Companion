...
import { setLogger as setAstLogger } from "@plexcord-companion/ast-parser";
import { SourcePatch } from "@plexcord-companion/plexcord-ast-parser";
import { setLogger as setWebpackLogger, WebpackAstParser } from "@plexcord-companion/webpack-ast-parser";

import { onEditCallback, onOpenCallback, reloadDiagnostics } from "@ast/plexcord/diagnostics";
import { I18nHover } from "@ast/plexcord/hover";
import { PatchCodeLensProvider, PluginDefCodeLensProvider, WebpackCodeLensProvider } from "@ast/plexcord/lenses";
...
export async function activate(context: ExtensionContext) {
    ...
    context.subscriptions.push(
        window.registerTreeDataProvider("plexcordSettings", new treeDataProvider()),
        workspace.onDidChangeTextDocument(onEditCallback),
        workspace.onDidOpenTextDocument(onOpenCallback),

        workspace.registerTextDocumentContentProvider("plexcord-companion", {
        ...
        commands.registerCommand("plexcord-companion.openPatchHelper", async (doc: TextDocument, patch: SourcePatch) => {
            if (!doc) {
        ...
        commands.registerCommand("plexcord-companion.diffModule", async (args) => {
            if (args) {
                try {
        ...
        commands.registerCommand("plexcord-companion.diffModuleSearch", async (args: string, findType: "string" | "regex") => {
            if (args) {
                try {
                    const r = await sendAndGetData<"diff">({
                        type: "diff",
        ...
        commands.registerCommand("plexcord-companion.extractFind", async (args: Discriminate<OutgoingMessage, "extract">) => {
            if (!args) {
        ...
        commands.registerCommand("plexcord-companion.extract", async (args: number) => {
            if (args) {
                try {
        ...
        commands.registerCommand("plexcord-companion.extractSearch", async (args: string, findType: "string" | "regex") => {
            if (args) {
                try {
                    const r = await sendAndGetData<"extract">({
                        type: "extract",
        ...
        commands.registerCommand("plexcord-companion.disablePlugin", async (data: DisablePluginData) => {
            try {
                if (!data)
...
async function isPlexcordRepo(): Promise<boolean> {
    ...
    try {
        ...
        return remotes.includes("plexcord");
    } catch (e) {
        outputChannel.warn("isPlexcordRepo: error running git remote -v", e);
        return false;
    }
}
