...
import { compareVersions, SemVerVersion } from "@plexcord-companion/shared/util";
import { formatModule, ModuleDep, WebpackAstParser } from "@plexcord-companion/webpack-ast-parser";
...
class _ModuleCache {
    ...
    private async writeModules(modmap: Record<string, string>) {
        if (await exists(this.modpath)) {
            throw new Error(".modules already exists, please run `plexcord-companion.clearCache` first");
        }
...
export class ModuleDepManager {
    ...
    // FIXME: setting to start caching when a webpack module is opened / when the plexcord workspace is opened
    ...
    private async tryReadCache(): Promise<undefined | CacheData> {
        ...
        if (IS_TEST === false && this.useCache && await exists(cacheFile)) {
            ...
            if (!("companionVersion" in cachedData) || compareVersions(cachedData.companionVersion, SERVER_VERSION_FROM_BUILD) !== 0) {
                console.info("Removing _cache.json generated with an old version of PlexcordCompanion oldVersion: ", cachedData.companionVersion);
                rm(cacheFile)
                    .catch((e) => {
