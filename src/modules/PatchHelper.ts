...
import { PlexcordAstParser, SourcePatch } from "@plexcord-companion/plexcord-ast-parser";
...
// uri format: plexcord-patchhelper://patch/:id.js
export class PatchHelper {
    ...
    private constructor(private readonly doc: TextDocument, lastPatch: SourcePatch) {
        this.id = nanoid();
        this.ast = new PlexcordAstParser(doc.getText(), doc.fileName);
        this.lastPatch = lastPatch;
    ...
    private onChange(newAst: PlexcordAstParser) {
        this.ast = newAst;
    ...
    public static onCloseDocument(e: TextDocument) {
        if (e.uri.scheme === "plexcord-patchhelper") {
    ...
    public static changeDocument({ document }: TextDocumentChangeEvent) {
        try {
            ...
            const newAst = new PlexcordAstParser(document.getText(), document.fileName);
    ...
    public static changeActiveEditor(_editor: TextEditor | undefined): void {
        if (
            window.activeTextEditor?.document.uri.scheme === "plexcord-patchhelper"
        ) {
    ...
    public static register({ subscriptions }: ExtensionContext) {
        ...
        subscriptions.push(workspace.registerTextDocumentContentProvider("plexcord-patchhelper", this));
    }
}
