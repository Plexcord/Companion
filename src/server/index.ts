...
import { reloadDiagnostics } from "@ast/plexcord/diagnostics";
...
const USERPLUGIN_LINK = "https://github.com/MutanPlex/Plexcord/blob/main";
    ...
    wss.on("connection", (sock, req) => {
        ...
        sock.on("message", (msg) => {
            try {
                ...
                switch (rec.type) {
                    ...
                    case "report": {
                        window.showErrorMessage("The Reporter feature is now removed from Plexcord Companion");
                        break;
                    }
        ...
        setTimeout(async () => {
            try {
                ...
                if (isOutdated) {
                    activeSocket?.close(CloseCode.OUTDATED_CLIENT, "Client is outdated, please update from https://github.com/MutanPlex/Plexcord/blob/main");
                    window.showErrorMessage("Plexcord Companion: Your client is out of date, please update your userplugin from https://github.com/MutanPlex/Plexcord/blob/main", "Open Link")
                        .then((button) => {
                            if (button === "Open Link") {
                ...
                window.showWarningMessage("Plexcord Companion: Unable to ensure client is up to date, you should update from https://github.com/MutanPlex/Plexcord/blob/main", "Open Link")
                    .then((button) => {
                        if (button === "Open Link") {
                            commands.executeCommand("vscode.open", USERPLUGIN_LINK);
                        }
